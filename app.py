...
from intelligent_chatbot import get_chatbot
import numpy as np
...
@st.cache_resource
def get_chatbot():
    """Get chatbot instance with caching"""
    from intelligent_chatbot import get_chatbot as get_intelligent_chatbot
    return get_intelligent_chatbot()
...
def assess_usage_with_ai(water_gallons, electricity_kwh, gas_cubic_m, user=None):
    ...
    return {
        ...
        'ai_points': round(ai_points, 1),  # Add both keys for compatibility
...
def calculate_ai_points(water_status, electricity_status, gas_status, efficiency_score, user=None):
    """AI-based points calculation allowing decimal and negative values for realistic ranking"""
    # Enhanced scoring system allowing full range including negatives
    status_base_scores = {
        'Excellent': 4.5,
        'Very Good': 3.8,
        'Good': 3.2,
    ...
    # Calculate total from three utilities (allowing negative scores)
    total_utility_points = water_points + electricity_points + gas_points
    
    # Apply efficiency modifier (can reduce or enhance scores)
    if efficiency_score >= 95:
        efficiency_modifier = 1.15  # Bonus for exceptional efficiency
        ...
        efficiency_modifier = 0.25  # Severe penalty for very poor efficiency
    
    # Apply efficiency modifier
    modified_points = total_utility_points * efficiency_modifier
    
    # Context adjustments (realistic bonuses/penalties)
    context_adjustment = 0.0
    if user:
        # Household efficiency adjustment
        household_size = getattr(user, 'adults', 1) + getattr(user, 'children', 0) + getattr(user, 'seniors', 0)
        if household_size > 4:
            context_adjustment += 0.8  # Bonus for larger households being efficient
        elif household_size == 1:
            context_adjustment -= 0.3  # Small penalty for single person high usage
        
        # Housing type adjustment
        housing_type = getattr(user, 'housing_type', '').lower()
        if 'apartment' in housing_type:
            context_adjustment += 0.5  # Apartments typically more efficient
        elif 'house' in housing_type:
            context_adjustment -= 0.2  # Houses typically use more utilities
    
    # Calculate final points (no artificial limits)
    final_points = modified_points + context_adjustment
    
    # Allow full range including negatives for proper ranking
    return round(final_points, 2)  # Two decimal places for precision
...
def smart_assistant(material):
    ...
    result = {
        'ai_sustainability_score': ai_analysis.get('sustainability_score', 50),
        'environmental_impact': ai_analysis.get('environmental_impact', 'Moderate'),
        'recyclability_score': ai_analysis.get('recyclability_score', ai_analysis.get('recyclability', 50)),
        'material_category': ai_analysis.get('category', 'Unknown'),
    ...
    **Note:** 
    ...
    if assess_button or save_button or history_button:
        ...
        try:
            ...
            ai_points = ai_analysis.get('ai_points', ai_analysis.get('points', calculate_ai_points(water_status, electricity_status, gas_status, efficiency_score)))
        except Exception as e:
            ...
            if save_button:
                with st.spinner("Checking daily limits and saving data..."):
                    ...
                    if not can_save:
                        ...
                        ai_points = ai_analysis.get('points', 5.0) if ai_analysis else 5.0
        ...
        if ai_analysis:
            ...
            with col1:
                ai_points = ai_analysis.get('ai_points', ai_analysis.get('points', 5.0))
    ...
    if data_for_analysis and len(data_for_analysis) > 3:
        ...
        if recommendations:
            for i, rec in enumerate(recommendations):
                if isinstance(rec, dict):
                    with st.expander(f"Recommendation {i+1}: {rec.get('category', 'General')} ({rec.get('priority', 'Medium')} Priority)"):
                        st.write(rec.get('message', str(rec)))
                        if 'potential_savings' in rec:
                            st.success(f"Potential Savings: {rec['potential_savings']}")
                        if 'impact' in rec:
