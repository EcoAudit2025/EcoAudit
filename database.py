...
class User(Base):
    ...
    email = Column(String(100), nullable=False)  # Email is now required for all accounts
    ...
    total_points = Column(Float, default=0.0)  # Total points earned
...
def create_user(username, email, location_type=None, climate_zone=None, 
    ...
    try:
        ...
        # Welcome message will be shown in UI
...
def find_user_by_username_or_email(identifier):
    ...
    try:
        ...
        if not user:
            # Try email
            user = session.query(User).filter(User.email == identifier.strip().lower()).first()
        return user
    except Exception as e:
        print(f"Error finding user: {e}")
        return None

def reset_user_security_code(username_or_email, new_confirmation_code):
    """Reset user's security code after verification"""
    try:
        user = find_user_by_username_or_email(username_or_email)
        if user and validate_confirmation_code(new_confirmation_code)[0]:
            user.confirmation_code = new_confirmation_code
            session.commit()
            return True
        return False
    except Exception as e:
        print(f"Error resetting security code: {e}")
        return False

def verify_household_information(user_email, adults, children, seniors, household_type):
    """Verify household information for security code reset"""
    try:
        user = get_user_by_email(user_email)
        if user:
            return (
                user.adults == adults and
                user.children == children and
                user.seniors == seniors and
                user.household_type == household_type
            )
        return False
    except Exception as e:
        print(f"Error verifying household information: {e}")
        return False
...
def save_utility_usage_to_history(user_id, water_gallons, electricity_kwh, gas_cubic_m, water_status, electricity_status, gas_status, efficiency_score=None, carbon_footprint=None):
    """Save utility usage data to history without points or daily limits"""
    session = get_session()
    try:
        usage = UtilityUsage(
            user_id=user_id,
            ...
            efficiency_score=efficiency_score,
            carbon_footprint=carbon_footprint,
            points_earned=0  # No points for history entries
        )
        session.add(usage)
        session.commit()
        
        return True, "Data saved to history successfully! (No points awarded)"
        
    except Exception as e:
        session.rollback()
        return False, f"Error saving to history: {e}"
    finally:
        session.close()
...
def update_user_points(user_id, points_to_add):
    ...
    try:
        user = session.query(User).filter(User.id == user_id).first()
        if user:
            current_points = user.total_points if user.total_points is not None else 0.0
...
def delete_user_account_permanently(user_id, admin_code=None):
    ...
    try:
        ...
        # Get user information before deletion for confirmation
        user = session.query(User).filter(User.id == user_id).first()
        if not user:
            return False, "User not found."
        ...
        # Delete all associated utility usage records
        session.query(UtilityUsage).filter(UtilityUsage.user_id == user_id).delete()
        
        # Delete all associated recycling verifications
        session.query(RecyclingVerification).filter(RecyclingVerification.user_id == user_id).delete()
        ...
        return True, f"Account '{username}' ({account_type}) and all associated data permanently deleted."
...
def reset_user_points_and_rankings(reset_code):
    """Reset only user points and global monitor rankings while preserving all user data and history
    Requires special reset code for security
    """
    # Security check - only allow reset with correct code
    VALID_RESET_CODE = "Atishay,Adit,Akshaj@EcoAudit_Team"
    ...
    try:
        ...
        # Reset only user points while keeping all other data
        session.query(User).update({
            User.total_points: 0,
            User.environmental_class: None  # Reset environmental class
        })
        
        # Reset points in utility usage records while keeping the records
        session.query(UtilityUsage).update({
            UtilityUsage.points_earned: 0
        })
        
        # Reset points in recycling verifications while keeping the records
        session.query(RecyclingVerification).update({
            RecyclingVerification.points_awarded: 0
        })
        ...
        return True, f"User points and global monitor rankings successfully reset! All users and their history have been preserved. Only points reset to 0. Reset performed at {datetime.now()}."
    except Exception as e:
        session.rollback()
        session.close()
        return False, f"Error resetting user points: {str(e)}"
